...
def readconfigfile(path) -> tuple[dict, dict]:
    ...
    try:
        ...
        logging.critical("[INIT] Config file malformed: Error while reading Tokens section! The file may be missing or malformed.")
        exit(1)
    ...
    try:
        ...
        logging.critical("[INIT] Config file malformed: Error while reading Options section! The file may be missing or malformed.")
        exit(1)
...
async def on_ready():
    ...
    if bot.options['sync-on-start']:
        ...
        logging.info("[INIT] Skipping command tree sync. Please manually sync commands later.")
...
try:
    ...
    logging.critical("[FATAL] Bot is missing the Message Content and/or Server Members intent! Please enable it in the Discord Developers web portal. Exiting...")
    exit()
