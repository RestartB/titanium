...
# Song parse function
async def song(self, item: spotipy.Spotify.track, interaction: discord.Interaction, add_button_url: str = None, add_button_text: str = None, cached: bool = False, ephemeral: bool = False, msg_id: int = None):
    """
    ...
    # Set up new embed
    embed = discord.Embed(title = f"{item['name']}{f' {self.bot.options['explicit-emoji']}' if explicit else ''}", description=f"on **[{await escape(item['album']['name'])}](<{item['album']['external_urls']['spotify']}>) • {item['album']['release_date'].split('-', 1)[0]}**", color = Color.from_rgb(r = 255, g = 255, b = 255))
    ...
    try:
        ...
        # Send new embed
        webhook = await interaction.followup.send(embed = embed, view = view_instance, ephemeral=ephemeral, wait=True)
...
async def artist(self, item: spotipy.Spotify.artist, top_tracks: spotipy.Spotify.artist_top_tracks, interaction: discord.Interaction, add_button_url: str = None, add_button_text: str = None, ephemeral: bool = False, msg_id: int = None):
    """
...
async def album(self, item: spotipy.Spotify.album, interaction: discord.Interaction, add_button_url: str = None, add_button_text: str = None, cached: bool = False, ephemeral: bool = False, msg_id: int = None):
    """
