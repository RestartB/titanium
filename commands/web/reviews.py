    ...
    async def serverReviews(self, interaction: discord.Interaction, server_id: str = "", ephemeral: bool = False):
        ...
        try:
            if interaction.guild is None:
                if server_id == "":
                    embed = discord.Embed(title = "Error", description = "Please specify a server ID.", color = Color.red())
                    embed.set_footer(text = f"@{interaction.user.name}", icon_url = interaction.user.display_avatar.url)
                    ...
                    try:
                        ...
                        embed = discord.Embed(title = "Error", description = "Invalid server ID.", color = Color.red())
                        embed.set_footer(text = f"@{interaction.user.name}", icon_url = interaction.user.display_avatar.url)

                        await interaction.followup.send(embed = embed, ephemeral=ephemeral)
                        return
                    except discord.errors.NotFound:
                        embed = discord.Embed(title = "Error", description = "Server not found.", color = Color.red())
                        embed.set_footer(text = f"@{interaction.user.name}", icon_url = interaction.user.display_avatar.url)
            ...
            request_url = f"https://manti.vendicated.dev/api/reviewdb/users/{guild.id}/reviews"
