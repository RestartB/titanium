...
class leaderboard(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.connection = sqlite3.connect(
            f"{self.bot.path}{self.bot.pathtype}content{self.bot.pathtype}sql{self.bot.pathtype}lb.db"
        )
            ...
            if message.author.bot != True:
                ...
                if self.cursor.execute(
                    ...
                    if self.cursor.execute(
                        ...
                        self.cursor.execute(
                            f"UPDATE '{message.guild.id}' SET messageCount = messageCount + 1, wordCount = wordCount + {len((message.content).split())} WHERE userMention = '{message.author.mention}'"
                        )
                    else:
                        # User is not on leaderboard, add them to the leaderboard
                        self.cursor.execute(
                            f"INSERT INTO '{message.guild.id}' (userMention, messageCount, wordCount) VALUES ('{message.author.mention}', 1, {len((message.content).split())})"
                        )
            ...
            if self.cursor.execute(
                ...
                class Leaderboard(View):
                    ...
                    async def prev_button(self,
                        ...
                        embed.set_footer(
                            text=
                            f"Requested by {interaction.user.name} - Page {self.page + 1}/{len(self.pages)}",
                            icon_url=interaction.user.avatar.url)
                ...
                embed.set_footer(
                    text=
                    f"Requested by {interaction.user.name} - Page 1/{len(pages)}",
                    icon_url=interaction.user.avatar.url)
                ...
                if len(pages) == 1:
                    ...
                    await interaction.edit_original_response(
                        embed=embed, view=Leaderboard(pages))
            else:
                embed = discord.Embed(
                    title="Not Enabled",
                    description=
                    "The message leaderboard is not enabled in this server.",
                    color=Color.red())
                await interaction.edit_original_response(embed=embed)
        except Exception:
            embed = discord.Embed(
                ...
                description=
                "Please try again later or message <@563372552643149825> for assistance.",
                color=Color.red())
            await interaction.edit_original_response(embed=embed, view=None)

    lbGroup = app_commands.Group(name="lb-control",
                                 description="Control the leaderboard.")

    # Enable LB command
    @lbGroup.command(name="enable",
                     description="Enable the message leaderboard.")
    ...
    async def enable_lb(self, interaction: discord.Interaction):
        ...
        try:
            if self.cursor.execute(
                ...
                self.cursor.execute(
                    f"CREATE TABLE '{interaction.guild.id}' (userMention text, messageCount integer, wordCount integer)"
                )
                embed = discord.Embed(
                    title="Success",
    ...
    # Disable LB command
    @lbGroup.command(name="disable",
                     description="Disable the message leaderboard.")
    @app_commands.checks.has_permissions(administrator=True)
    async def disable_lb(self, interaction: discord.Interaction):
            ...
            try:
                if self.cursor.execute(
                    ...
                    embed = discord.Embed(
                        title="Failed",
                        description=
                        "Leaderboard is already disabled in this server.",
                        color=Color.red())
                    await interaction.edit_original_response(embed=embed)
                else:
                    self.cursor.execute(f"DROP TABLE '{interaction.guild.id}'")
                    embed = discord.Embed(title="Disabled.",
                                          color=Color.green())
                    await interaction.edit_original_response(embed=embed)
            except Exception:
                embed = discord.Embed(
        ...
        view = View()
        delete_button = discord.ui.Button(label='Delete',
        ...
        embed = discord.Embed(
            title="Are you sure?",
            description=
            "The leaderboard will be disabled, and data for this server will be deleted!",
            color=Color.orange())
        await interaction.followup.send(embed=embed, view=view, ephemeral=True)

    # Reset LB command
    @lbGroup.command(name="reset",
                     description="Resets the message leaderboard.")
    @app_commands.checks.has_permissions(administrator=True)
    async def reset_lb(self, interaction: discord.Interaction):
        ...
        if self.cursor.execute(
            ...
            async def delete_callback(interaction: discord.Interaction):
                ...
                embed = discord.Embed(title="Resetting...",
                                      color=Color.orange())
                await interaction.edit_original_response(embed=embed,
                                                         view=None)
                ...
                try:
                    self.cursor.execute(
                        f"DELETE FROM '{interaction.guild.id}';")
            ...
            view = View()
            delete_button = discord.ui.Button(label='Reset',
                                              style=discord.ButtonStyle.red)
            delete_button.callback = delete_callback
            view.add_item(delete_button)
            ...
            embed = discord.Embed(
                title="Are you sure?",
                description=
                "The leaderboard will be reset and all data will be removed!",
                color=Color.orange())
            await interaction.edit_original_response(embed=embed, view=view)

    # Privacy command
    @lbGroup.command(name="privacy",
                     description="View the leaderboard privacy disclaimer.")
