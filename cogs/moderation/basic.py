...
class ModerationBasicCog(
    ...
    def _purge_check(
        self, message: discord.Message, target: discord.User | None, bot_only: bool
    ) -> bool:
    ...
    async def _warn_member(
        self, interaction: discord.Interaction["TitaniumBot"], member: discord.Member, reason: str
        ...
        try:
            ...
            if not isinstance(interaction.user, discord.Member) or not self._hierarchy_check(
                member, interaction.user, interaction
            ):
                return PunishmentResult.NOT_ALLOWED, None, None, None

            # Check if member is already being punished
            if (
                interaction.guild.id in self.bot.punishing
            ...
            self.bot.punishing.setdefault(interaction.guild.id, []).append(member.id)

            # Create case
            async with get_session() as session:
                manager = GuildModCaseManager(self.bot, interaction.guild, session)

                case, dm_success, dm_error = await manager.create_case(
                    action=CaseType.WARN,
                    user=member,
                    creator_user=interaction.user,
                    reason=reason,
                )
    ...
    async def _ban_member(
        ...
        try:
            ...
            member = await get_or_fetch_member(self.bot, interaction.guild, user.id)

            # Check if target doesn't have higher role
            if not isinstance(interaction.user, discord.Member) or (
                isinstance(member, discord.Member)
                and not self._hierarchy_check(member, interaction.user, interaction)
            ):
                return PunishmentResult.NOT_ALLOWED, None, None, None

            # Check if Titanium can punish target
            if isinstance(member, discord.Member) and not self._bot_perms_check(
                member, interaction
            ):
                return PunishmentResult.BOT_NOT_ALLOWED, None, None, None
            ...
            if (
                interaction.guild.id in self.bot.punishing
                and user.id in self.bot.punishing[interaction.guild.id]
            ):
                return PunishmentResult.ALREADY_PUNISHING, None, None, None

            # Add member to punishing list
            self.bot.punishing.setdefault(interaction.guild.id, []).append(user.id)
            ...
            async with get_session() as session:
                ...
                try:
                    await interaction.guild.ban(
                        user=user,
                        reason=f"@{interaction.user.name}: {processed_reason}",
    ...
    async def unmute(
        ...
        try:
            ...
            # Unmute user
            try:
                await member.timeout(None, reason=f"Unmuted by @{interaction.user.name}")
            except discord.Forbidden as e:
                await log_error(
                    bot=self.bot,
                    module="Moderation",
    ...
    async def purge(
        ...
        try:
            ...
            deleted = await interaction.channel.purge(
                limit=amount,
                before=discord.Object(id=interaction.id),
                bulk=True,
                reason=f"Purged by @{interaction.user.name}",
                check=lambda m: self._purge_check(m, user, bot_only),
            )
        except discord.Forbidden as e:
            if not isinstance(
                interaction.channel,
                ...
                await log_error(
                    ...
                    module="Moderation",
                    guild_id=interaction.guild.id,
                    error=f"Titanium was not allowed to purge messages in #{interaction.channel.name if interaction.channel else 'Unknown'} ({interaction.channel.id if interaction.channel else '???'})",
                    details=e.text,
                    exc=e,
                )
            ...
            if not isinstance(
                ...
                await log_error(
                    ...
                    module="Moderation",
                    guild_id=interaction.guild.id,
                    error=f"Unknown Discord error while purging messages in #{interaction.channel.name if interaction.channel else 'Unknown'} ({interaction.channel.id if interaction.channel else '???'})",
                    details=e.text,
                    exc=e,
                )
    ...
    async def masswarn(
        ...
        for user in valid_users:
            status, _, dm_success, dm_error = await self._warn_member(
                interaction=interaction, member=user, reason=reason
            )

            if status == PunishmentResult.SUCCESS:
                successful_warns.append((user, dm_error or "DM Error" if not dm_success else ""))
    ...
    async def massmute(
        ...
        for user in valid_users:
            status, _, dm_success, dm_error = await self._mute_member(
                interaction=interaction, member=user, duration=duration, reason=reason
            )

            if status == PunishmentResult.SUCCESS:
                successful_mutes.append((user, dm_error or "DM Error" if not dm_success else ""))
            elif status == PunishmentResult.NOT_IN_GUILD:
    ...
    async def masskick(
        ...
        for user in valid_users:
            ...
            if status == PunishmentResult.SUCCESS:
                successful_kicks.append((user, dm_error or "DM Error" if not dm_success else ""))
            elif status == PunishmentResult.NOT_IN_GUILD:
    ...
    async def massban(
        ...
        async with get_session() as session:
            ...
            try:
                ...
                ban_result = await interaction.guild.bulk_ban(
                    users=valid_users,
                    reason=f"@{interaction.user.name}: {processed_reason}",
